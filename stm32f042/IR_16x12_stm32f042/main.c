...
static int16_t   ir_page_0[12][16];  //valuse of IR sensors subpage page 0
 ...
 while ( 1 ) 
  {	
	...
	for (i=0; i<6; i++)
	{
	  ...
	  for (j=0; j<16; j++)
	  {
	    ir_page_0[2*i][j] = (array[2*j]<<8)|array[1+2*j];
	    ir_page_0[2*i+1][j] = (array[32+2*j]<<8)|array[33+2*j];
	  }
	}
	// Give back ir_mutex
