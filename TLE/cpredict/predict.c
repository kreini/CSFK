...
struct sat
{
 ...
 int 		catnum;			/* Catalog Number (a.k.a NORAD id) */
 int 		setnum;          	/* Element Set No. */
...
#ifdef	PREDICT_HAVE_CALLSIGN
 char		callsign[17];	/* Observation Position Call Sign	*/
#endif
};

static	struct	sat	sat;
static	struct	qth	qth;

/* Global variables for sharing data among functions... */
static	double	tsince, jul_epoch, jul_utc, eclipse_depth=0,
...
static	char	/*qthfile[50], tlefile[50], dbfile[50],*/ temp[80], /* output[25], */
	ephem[5], sat_sun_status, findsun, calc_squint;

static	int	indx, iaz, iel, ma256, isplat, isplong, Flags=0;
...
static double Degrees(double arg)
{
	/* Returns angle in degrees from argument in radians */
	return (arg/deg2rad);
}

static double ArcSin(double arg)
{
...
static double ArcCos(double arg)
{
	/* Returns arccosine of argument */
	return(pio2-ArcSin(arg));
}

static void Magnitude(vector_t *v)
{
...
/*
static void Vec_Add(vector_t *v1, vector_t *v2, vector_t *v3)
{
	v3->x=v1->x+v2->x;
	v3->y=v1->y+v2->y;
	v3->z=v1->z+v2->z;
...
static void Vec_Sub(vector_t *v1, vector_t *v2, vector_t *v3)
{
...
static void Scalar_Multiply(double k, vector_t *v1, vector_t *v2)
{
...
static void Scale_Vector(double k, vector_t *v)
{
...
static double Dot(vector_t *v1, vector_t *v2)
{
...
static void Cross(vector_t *v1, vector_t *v2 ,vector_t *v3)
{
...
/*static double Int(double arg)
{
	return(floor(arg));
}
*/

static void Convert_Sat_State(vector_t *pos, vector_t *vel)
{
...
static double Julian_Date_of_Epoch(double epoch)
{
...
/*
static int DOY (int yr, int mo, int dy)
{
...
/*
static double Julian_Date(struct tm *cdate)
{

	double julian_date;

	julian_date=Julian_Date_of_Year(cdate->tm_year)+DOY(cdate->tm_year,cdate->tm_mon,cdate->tm_mday)+Fraction_of_Day(cdate->tm_hour,cdate->tm_min,cdate->tm_sec)+5.787037e-06; 
...
/*
static void Date_Time(double julian_date, struct tm *cdate)
{
...
static void Calculate_Solar_Position(double time, vector_t *solar_vector)
{
...
static int Sat_Eclipsed(vector_t *pos, vector_t *sol, double *depth)
{
...
static void select_ephemeris(tle_t *tle)
{
...
static void SGP4(double tsince, tle_t * tle, vector_t * pos, vector_t * vel)
{
...
static void Deep(int ientry, tle_t * tle, deep_arg_t * deep_arg)
{
...
static void SDP4(double tsince, tle_t * tle, vector_t * pos, vector_t * vel)
{
	/* This function is used to calculate the position and velocity */
...
static void Calculate_User_PosVel(double time, geodetic_t *geodetic, vector_t *obs_pos, vector_t *obs_vel)
{
...
static void Calculate_LatLonAlt(double time, vector_t *pos,  geodetic_t *geodetic)
{
...
static void Calculate_Obs(double time, vector_t *pos, vector_t *vel, geodetic_t *geodetic, vector_t *obs_set)
{
...
/*
static void Calculate_RADec(double time, vector_t *pos, vector_t *vel, geodetic_t *geodetic, vector_t *obs_set)
{
...
static double PrimeAngle(double x)
{
...
/*
static void CopyString(char *source, char *destination, int start, int end)
{
...
/*
static char *Abbreviate(char *string, int n)
{
...
static int KepCheck(char *line1, char *line2)
{
...
static void InternalUpdate(int x)
{
...
/*
static char *noradEvalue(double value)
{
...
static void Data2TLE(int x)
{
	...
	sprintf(string,"%5i",sat.orbitnum);
...
/*
static double ReadBearing(char *input)
{
...
static int ReadTLE(char *line0, char *line1, char *line2)
{
	int la, lb, lc;
	int error_flags,a,b,c,d;
...
/*
static int ReadQTH(double lat, double lon, long alt)
{
...
/*
static char *Daynum2String(double daynum)
{
...
static void FindMoon(double daynum)
{
...
/*
static void FindSun(double daynum)
{
...
static void Calc()
{
	...
	irk=(long)floor(sat_range+0.5);
	isplat=(int)floor(sat_lat+0.5);
	isplong=(int)floor(360.0-sat_lon+0.5);
	iaz=(int)floor(sat_azi+0.5);
	iel=(int)floor(sat_ele+0.5);
	ma256=(int)floor(256.0*(phase/twopi)+0.5);

	if (sat_sun_status)
	{
		if (sun_ele<=-12.0 && floor(sat_ele-0.5)>=0.0)
		{
			findsun='+';
		}
		else
...
static char Decayed(int x, double time)
{
...
static char Geostationary(int x)
{
...
*/
static int MakeObservation(double obs_time, struct observation * obs) 
{
...
{
 fprintf(fw,"NORAD_ID        %d\n", (int)obs->norad_id);
 fprintf(fw,"Name            %s\n", obs->name);
 fprintf(fw,"Date(epoch)     %f\n", obs->epoch);
 fprintf(fw,"Latitude(N)     %f\n", obs->latitude);
 fprintf(fw,"Longitude(E)    %f\n", obs->longitude);
 fprintf(fw,"Azimuth         %f\n", obs->azimuth);
 fprintf(fw,"Elevation       %f\n", obs->elevation);
 fprintf(fw,"Velocity        %f\n", obs->orbital_velocity);
 fprintf(fw,"Footprint       %f\n", obs->footprint);
 fprintf(fw,"Altitude(km)    %f\n", obs->altitude);
 fprintf(fw,"Slant_Range(km) %f\n", obs->slant_range);
 fprintf(fw,"Eclipse_Depth   %f\n", obs->eclipse_depth);
 fprintf(fw,"Orbital_Phase   %f\n", obs->orbital_phase);
 fprintf(fw,"Orbital_Model   %s\n", obs->orbital_model);
 fprintf(fw,"Visibility      %c\n", obs->visibility);
 fprintf(fw,"Sunlight        %d\n", obs->sunlit);
 fprintf(fw,"Orbit_Number    %d\n",obs->orbit);
 fprintf(fw,"Geostationary   %d\n", obs->geostationary);
 fprintf(fw,"AOS_Happens     %d\n", obs->has_aos);
 fprintf(fw,"Decayed         %d\n", obs->decayed);
 fprintf(fw,"Doppler         %f\n", obs->doppler);
 fprintf(fw,"ECI_x           %f\n", obs->eci_x);
 fprintf(fw,"ECI_y           %f\n", obs->eci_y);
 fprintf(fw,"ECI_z           %f\n", obs->eci_z);
 fprintf(fw,"ECI_vx          %f\n", obs->eci_vx);
 fprintf(fw,"ECI_vy          %f\n", obs->eci_vy);
 fprintf(fw,"ECI_vz          %f\n", obs->eci_vz);
 fprintf(fw,"ECI_sun_x       %f\n", obs->eci_sun_x);
 fprintf(fw,"ECI_sun_y       %f\n", obs->eci_sun_y);
 fprintf(fw,"ECI_sun_z       %f\n", obs->eci_sun_z);
 fprintf(fw,"ECI_obs_x       %f\n", obs->eci_obs_x);
 fprintf(fw,"ECI_obs_y       %f\n", obs->eci_obs_y);
 fprintf(fw,"ECI_obs_z       %f\n", obs->eci_obs_z);
 fprintf(fw,"Beta_Angle(deg) %f\n", obs->beta_angle);
 return(0);
}

static char load(char *tle0,char *tle1,char *tle2,double epoch,double stnlat,double stnlong,double stnalt)
{
...
int predict_observation(char *tle0,char *tle1,char *tle2,double epoch,double stnlat,double stnlong,double stnalt,struct observation *obs)
{
 if ( obs==NULL )
	return(-1);
 else if ( load(tle0,tle1,tle2,epoch,stnlat,-stnlong,stnalt) != 0 )
	return(-2);
 else if ( MakeObservation(daynum,obs) != 0 )
 	return(-3);
 else
	return(0);
}
