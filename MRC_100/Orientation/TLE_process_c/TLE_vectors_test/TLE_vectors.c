...
int main() {
	...
	while(1) {
		fprintf(fw, "splot [-1:1] [-1:1] [-1:1] 'tmp.txt' using 1:2:3 with lines notitle, 'tmp.txt' every ::1::1 using 1:2:3:(sprintf(\"sat\")) with labels point pt 7 offset char 1, 1, 1 notitle, 'tmp.txt' every ::3::3 using 1:2:3:(sprintf(\"Sun\")) with labels point pt 7 offset char 1, 1, 1 notitle, 'tmp.txt' every ::0::0 using 1:2:3:(sprintf(\"Earth\")) with labels point pt 7 offset char 1, 1, 1 notitle\n");
		fflush(fw);
		msleep(100);
	}
