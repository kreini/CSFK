...
void readSerial_task(void *parameters) {
 	char	buff[32];
	...
	while ( 1 ) 
  	{	
  		stm32_gpio_output_toggle(GPIOB,3);
  		printf("inwhile loop readSerial\n");
  		fprintf(stdout,"[%d]\n",i);
	 	msleep(500);

  		printf("%d\n",i);
  		printf("kinwhile loop readSerial\n");
  		printf("mast");
  		msleep(500);
...
void thermometer_task(void * parameters) 
{
 ...
 while ( 1 ) 
  {	int	n,r;
	...
	r=stm32_i2c_write_read(I2C1,0x48,"\x00",1,array,2);
	temperature=(array[0]<<8)|array[1];

	n=sprintf(buff,"%d [0x%.4x]\n",r,(unsigned)temperature&0xFFFF);
