/* 6th step: RTOS and I2C with queue and struct and DMA interrupt with busy wait sampling */
/* STM32F072 */
...
void blink_task(void* parameters) 
{
	...
	while (1) {
		...
		addr[2] = 0x00;
		addr[3] = 0x00;
		stm32_i2c_dma_write_read(I2C2, 0x33, addr, 2, buff2, 64);
		usart_write_queue(USART2, buff2, 16);
	...
	*/
	...
	while (1) {
		...
		if (subpage == 0) {
			...
			i2c_tx.reg_addr = addr_sub0[k]; /* 64 bit, two lines in a row */
			i2c_tx.buffer = array;
		...
		/* reading subpage 1 */
			...
			i2c_tx.i2c_addr = 0x33;
			i2c_tx.reg_addr = addr_sub1[k]; /* 64 bit, two lines in a row */
			i2c_tx.buffer = array;
			i2c_tx.num_bytes = number_of_columns*2;
