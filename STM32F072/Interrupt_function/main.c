...
// timer delay handler, function and egyéb

void SysTick_Handler(unsigned ticks) {
	SysTick->LOAD = ticks;
	SysTick->VAL = 0;
...
static inline void timsleep(unsigned millisec) {
	SysTick_Handler(millisec * (F_CPU/8000L)); // HCLK: 24MHz
}
